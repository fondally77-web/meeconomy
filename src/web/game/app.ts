...
export class App {
  ...
  private intro(page = 0): void {
    const pages: { scene: Parameters<PipelineView['setScene']>[0]; text: string }[] = [
      ...
      { scene: 'farm', text: '羊は<b>✂️毛を刈れば、生きたまま なんども稼いでくれます</b>。<br>ただし飼っているあいだは毎月エサ代。<br><b>🔪お肉にすれば大きなお金が一度きり</b>——その子はもういません' },
      { scene: 'farm', text: 'つまり毎月の悩みはこれだけ。<br>「<b>この子は毛にする？ お肉にする？</b>」<br>どちらも正解で、どちらもちょっと痛い。それが経営です。メェ' },
  ...
  private stageFarm(): void {
    ...
    const strategy = m >= 3 && m <= 5
      ...
      : m >= 6 && m <= 8
        ...
        : '✂️毛はなんども・少しずつ。🔪お肉は一度きり・大きく。今月は片方に絞るとトラックの節約になります';
    this.texel(this.eventId === 'shearFes'
